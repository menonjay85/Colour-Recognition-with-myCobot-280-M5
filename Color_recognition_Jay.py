...
def take_inp():
    print("Please select a Bin to place the Cube : A/B/C/D")
    inp_place = input()
    inp_placeint = 2
    if inp_place == "D":
        inp_placeint = 0
...
class Object_detect():
    ...
    def move(self, x, y, color):
        ...
        inp_placeint = take_inp()
        self.mc.send_coords(self.move_coords[inp_placeint], 40, 1)
        time.sleep(3)
       ...
       # read camera
        ...
        if nparams < 10:
            ...
            detect.set_params(
                ...
                abs(detect.sum_x1-detect.sum_x2)/20.0 +
                abs(detect.sum_y1-detect.sum_y2)/20.0
            )
            print("ok")
            continue
